...
#         print(DTBS['E']['002455.SZ']['2023-04-21'])
    ...
    # 获取当日qs
    if DTBS['E'][s_code][td]['cl'] / DTBS['A'][code][td]['csp'] >= (qr * 0.01):
        DTBS['A'][code][td]['qs'] = 1
    else:
        DTBS['A'][code][td]['qs'] = 0

    # 获取当日xx
    if DTBS['E'][s_code][td]['cl'] / DTBS['A'][code][td]['csp'] < (xr * 0.01):
        DTBS['A'][code][td]['xx'] = 1
    else:
        DTBS['A'][code][td]['xx'] = 0
    ...
    # 获取强赎期限
    idx = len(DTBS['D']['day'])
    idx15 = [i for i in range(idx - qr1 + 1, idx)]
    idx30 = [i for i in range(idx - qr2 + 1, idx)]

    s = DTBS['A'][code][td]['qs']
    xx = DTBS['A'][code][td]['xx']
    ...
    # 获取下修期限
    idx = len(DTBS['D']['day'])
    idx15 = [i for i in range(idx - xr1 + 1, idx)]
    idx30 = [i for i in range(idx - xr2 + 1, idx)]
    ...
    # 更新xx15
    for j in idx15:
        if DTBS['A'][code][DTBS['D']['day'][j]]['xx'] == 1:
            xx += 1
        DTBS['A'][code][td]['xx15'] = xx
...
for code in new:
    ...
    if DTBS['E'][s_code][td]['cl'] / DTBS['A'][code][td]['csp'] >= (qr * 0.01):
        ...
        # 后期更改了，注意 by 张程功
        if 'qs15' in DTBS['A'][code][td].keys():
            DTBS['A'][code][td]['qs15'] = min(0, DTBS['A'][code][td]['qs15']-1)
        else:
            print(code)

        # 后期更改了，注意 by 张程功
        if 'qs30' in DTBS['A'][code][td].keys():
    ...
    if DTBS['E'][s_code][td]['cl'] / DTBS['A'][code][td]['csp'] < (xr * 0.01):
        ...
        # 后期更改，属于比较特殊的情况，第一天就触发了下修，因此直接暴力改动
        if 'xx15' not in DTBS['A'][code][td].keys():
            DTBS['A'][code][td]['xx15'] = 0
            DTBS['A'][code][td]['xx30'] = 0
        DTBS['A'][code][td]['xx15'] += 1
        DTBS['A'][code][td]['xx30'] += 1
    else:
        DTBS['A'][code][td]['xx'] = 0

        # 后期更改了，注意 by 张程功
        if 'xx15' in DTBS['A'][code][td].keys():
            DTBS['A'][code][td]['xx15'] = min(0, DTBS['A'][code][td]['xx15']-1)
        else:
            print(code)

        # 后期更改了，注意 by 张程功
        if 'xx30' in DTBS['A'][code][td].keys():
            DTBS['A'][code][td]['xx30'] = min(0, DTBS['A'][code][td]['xx30']-1)
        else:
            print(code)
